...
const SolflareWallet = () => {
  ...
  const [solflareWalletPublicKey, setSolflareWalletPublicKey] = useState<PublicKey>()
  ...
  const handleConnect = async () => {
    ...
    const url = buildUrl('solflare', 'v1/connect', params)
    Linking.openURL(url)
  }

  const handleShowBalance = async () => {
    try {
      const balance = parseInt(await connection.getBalance(solflareWalletPublicKey)) / 1e9
  ...
  const handleDisconnect = async () => {
    ...
    const url = buildUrl('solflare', 'v1/disconnect', params)
    Linking.openURL(url)
  }
  ...
  const handleSignMessage = async () => {
    ...
    const url = buildUrl('solflare', 'v1/signMessage', params)
    Linking.openURL(url)
  }
